...
from coach_agent import send_to_coach, get_workout_plan, clear_conversation
...
class CoachScreen(Screen):
    ...
    def update_user_context(self, user_data=None):
        ...
        if user_data:
            self.user_context = f"""You are a professional fitness coach. Your client's details are:
            Name: {user_data['name']}
            Age: {user_data['age']}
            ...
            Provide personalized fitness advice based on these details. Be encouraging and professional."""
            
            # If there's an active conversation, add a system message about the profile update
            if self.chat_history:
                self.chat_history.append({
                    'role': 'system',
                    'content': 'User profile has been updated. Adjusting advice accordingly.'
                })

    def get_workout_response(self, user_message):
        if self.user_context:
            ...
            response = get_workout_plan(messages)
            return response
    ...
    def setup_ui(self):
        ...
        self.user_input = TextInput(
            hint_text="Chat with your coach...",
            multiline=False,
            size_hint=(0.75, 1),
            padding=(dp(10), dp(10)),
        ...
        Clock.schedule_once(lambda dt: self.add_chat_message(
            "Hi! I'm your AI fitness coach. I'll help you create a personalized workout plan "
            "that matches your goals and experience level. Let's start by discussing your "
            "training goals. Are you looking to build muscle, gain strength, or focus on "
            "general fitness?",
            sender="Coach"
        ), 0.5)
    ...
    def add_chat_message(self, message, sender=""):
        ...
        bubble_color = (0.9, 0.9, 0.9, 1) if sender == "Coach" else (0.7, 0.9, 1, 1)
        align = 'left' if sender == "Coach" else 'right'
        prefix = "[b]Coach:[/b]\n" if sender == "Coach" else "[b]You:[/b]\n"
        ...
        def update_text_size(*args):
            ...
            label.pos = (dp(10), 0) if sender == "Coach" else (self.scroll.width - label.width - dp(10), 0)
        ...
        # Add message to chat (at the bottom)
    ...
    def handle_user_input(self, instance):
        ...
        def get_response(dt):
            response = self.get_workout_response(message)
            self.hide_typing_indicator()
            self.add_chat_message(response, sender="Coach")

            # Check if ready to generate plan
            if "ready to make plan" in response.lower():
    ...
    def generate_plan(self, instance):
        ...
        def create_plan(dt):
            plan = self.get_workout_response(user_data)
            self.hide_typing_indicator()
            self.add_chat_message(plan, sender="Coach")
    ...
    def export_to_pdf(self, instance):
        ...
        pdf.cell(200, 10, txt="Your Personalized Workout Plan", ln=1, align='C')
        ...
        if not os.path.exists('exports'):
            os.makedirs('exports')
        pdf_path = os.path.join('exports', 'workout_plan.pdf')
        pdf.output(pdf_path)
        
        # Show confirmation message
        self.add_chat_message(
            f"Your workout plan has been exported to: {pdf_path}",
            sender="Coach"
        )
